...
export default function FinishedTasks() {
    ...
    const fetchCategoryTasks = async (): Promise<TaskInList[]> => {
        const response = await apiFront.get(routes.todos.categoryTodosFinished(categoryId))
    ...
    return <div className="p-8 animate-fadeIn">
        <div className="max-w-4xl mx-auto">
            <div className="mb-8">
                <PageTitle title={(category?.title ? category.title + " - " : "") + "Finished" }/>
                <PageDescription description={category?.title ? `Законченые задачи категории ${category.title}` : "Законченые задачи категории"}/>
            </div>
